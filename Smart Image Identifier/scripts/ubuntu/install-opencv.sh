...
# Patch gen_java.py to generate Mat.release() instead of Mat.release() methods
sed -i 's/mat.release()/mat.release()/g' "$opencvhome$genjava"
...
# Patch utils+Converters.java to replace mi.release() with mi.release()
sed -i 's/mi.release()/mi.release()/g' "$opencvhome$converters"
